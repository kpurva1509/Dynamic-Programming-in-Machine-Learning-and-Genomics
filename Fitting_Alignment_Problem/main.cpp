...
string align(const int &match_score, const int &mismatch_mu, const int &indel_sigma, const string &s, const string &t)
{
    ...
    /* (n+1)x(m+1) tall-thin matrix */
    /* to fit string t into a substring of s */
    ...
    /* initialize first row to indel scores */
    for (int j = 1; j < m + 1; j++)
    {
    ...
    /* extract max value of last column */
    int max = -1*INF;
    int max_n;

    /* 'm' indicates index into last column */
    for(int i=0; i<n+1; i++) {
        if(longest_path[i][m] >= max) {
            max = longest_path[i][m];
            max_n = i;
        }
    }
